...
func TestNewAESCipher(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cipher, err := cipher.NewAESCipher(key)
			...
			if tt.wantErr != nil {
				...
				if cipher != nil {
					t.Error("NewAESCipher() should return nil cipher on error")
				}
			} else {
				if err != nil {
				...
				if cipher == nil {
					t.Error("NewAESCipher() should return non-nil cipher on success")
				}
			}
		})
	}
...
func TestAESCipher_EncryptDecryptRoundtrip(t *testing.T) {
	...
	for i, plaintext := range testCases {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			if len(plaintext) == 0 {
				// Skip empty plaintext as it returns an error
				return
			}
...
func TestAESCipher_EncryptionUniqueness(t *testing.T) {
	// Create a valid cipher
	key := make([]byte, config.KeySize)
	_, err := rand.Read(key)
	if err != nil {
...
func TestAESCipher_DifferentKeys(t *testing.T) {
	...
	c1, err := cipher.NewAESCipher(key1)
	if err != nil {
		t.Fatalf("failed to create cipher1: %v", err)
	}

	c2, err := cipher.NewAESCipher(key2)
	if err != nil {
